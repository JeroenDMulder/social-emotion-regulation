...
subtitle: "``Individual differences in developmental trajectories  of social emotion regulation from childhood to emerging adolescence'' by Mulder, Dobbelaar, and Achterberg (under review)"
author: 
...
To explore Aim 1, the behavioral and neural responses were analyzed using Bayesian multilevel growth curve models. For Aims 2 and 3, a single structural equation model was specified in which social well-being was predicted from the estimated individual growth components extracted from the Bayesian multilevel growth curve model. 

## Bayesian multilevel growth curve model (Aim 1)
The behavioral and neural measurements were analyzed using separate Bayesian multilevel growth curve models, and specified using the R package `brms` [@burkner_brms_2017] in R version 4.2.2 [@r_core_team_r_2019]. The Bayesian estimation framework was chosen because it can easily accommodate censoring by integrating out censored values (see @fig-cens) [@gelman_bayesian_2013]. Moreover, missing data can be imputed as part of the model fitting process under the assumption of missing at random (MAR) [@burkner_brms_2017]. The multilevel framework was chosen because it treats the time of observation as data (rather than as a parameter, as in a latent growth cNee, urve model), and can therefore more easily handle individually-varying times of observations [@mehta_putting_2000]. 
...
Social well-being data consists of 35 items, belonging to 1 of 5 subscales. The data were preprocessed by computing factor scores for each subscale by taking the mean of the respective items (as instructed in the subscale manuals). Using the R package *lavaan* (version 0.6.12), a 1-factor confirmatory factor analysis model was fitted with the subscales scores as indicators [@rosseel_lavaan_2012].
...
To take the uncertainty of the individual growth component estimates from the Bayesian multilevel growth curve models into account, the SEM model was fitted to each of the 1000 data sets using the R package *semTools* [@jorgensen_textttsemtools_2022]. Point and standard error estimates were pooled according to rules by @rubin_multiple_1987. 
...
#| echo: FALSE
#| fig-cap: "Model results for behavioural response."
#| warning: false
#| fig-width: 10
#| fig-height: 10
...
```{r}
#| label: fig-DLPFC-results
#| echo: FALSE
#| fig-cap: "Model results for DLPFC response."
#| warning: false
#| fig-width: 10
#| fig-height: 10
...
Results for the `AI` model are shown below:

```{r}
#| label: fig-results-AI
#| fig-cap: "Model results for AI response."
#| echo: FALSE
#| fig-width: 10
#| fig-height: 10
...
```{r}
#| label: fig-results-MPFC
#| fig-cap: "Model results for MPFC response."
#| echo: FALSE
#| fig-width: 10
#| fig-height: 10
